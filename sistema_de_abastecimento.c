...
const char HTML_BODY[] =
    ...
    "if(isNaN(minv)||isNaN(maxv)||minv<0||maxv>100||minv>maxv){fb('Valores invalidos','error');return;}"
    "fetch('/limites/min/'+minv+'/max/'+maxv).then(r=>r.text()).then(t=>fb(t,'ok'));"
    "min_in.value='';max_in.value='';}"
    "function fb(t,c){let e=document.getElementById('feedback');e.className='';e.classList.add(c);e.innerText=t;e.style.opacity=1;setTimeout(()=>{e.style.opacity=0;},3000);}"
